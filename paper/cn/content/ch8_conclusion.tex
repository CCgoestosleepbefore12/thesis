...
\textbf{贡献 3（实证）}：实现 B$+$D 双注入点编码器偏差注入，在 \texttt{pickup}、\texttt{wipe}、\texttt{pickandplace} 三个真机任务上完成 $4$ 列联合成功率矩阵评估（$2\times2$ “训练偏差 $\times$ 测试偏差”析因设计）。同时给出 HIL-SERL 真机失败的方法论级 negative result——SAC online 在小数据加稀疏奖励加 $10$\,Hz 真机吞吐下 actor 解冻后大幅漂移，为真机改用 BC $+$ HG-DAgger 提供实证支撑。

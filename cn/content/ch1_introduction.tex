...
\textbf{贡献 3：真机三任务在编码器偏差注入下的端到端验证}（\cref{sec:real_experiments}）。本文实现了 B$+$D 双注入点编码器偏差注入（\cref{subsec:bd_injection}），并在 \texttt{pickup}、\texttt{wipe}、\texttt{pickandplace} 三个真机任务上设计了 $4$ 列联合成功率矩阵评估（无 bias / 有 bias / $+$ backup 无 bias / $+$ backup 有 bias），构成本论文的核心实证主线（实验设计与 partial results 见\cref{sec:real_experiments}，完整矩阵随真机数据持续填充）。同时给出 HIL-SERL 真机失败的方法论级 negative result（详见\cref{subsec:hilserl_failure}），为真机改用 BC 加 HG-DAgger 的范式选择提供实证支撑。
